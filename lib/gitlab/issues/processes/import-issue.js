...
var Application = function(args) {
    ...
    function attachBoard(app, next) {
        ...
        db.Board.load(options, function(err, board) {
            if(err) return next(err);
            if(!board) return next(new Error("No board with that label exists"));
    ...
    function attachDeveloper(app, next) {
        var assignee = app.issue.assignee;

        if(!assignee) return next(null, app);

        var options = {
            criteria: { 'gitlab.id': assignee.id },
            select: 'id name'
        };
